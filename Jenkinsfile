      ...
      stage('Run Docker Image In Dev Server'){
        ...
        def dockerRun = ' docker run  -d -p 8083:8090 --name java-web-app kmounika11/java-web-app'
         ...
         sshagent(['DOCKER_SERVER']) {
          sh 'ssh -o StrictHostKeyChecking=no ubuntu@172.31.93.186 docker stop java-web-app || true'
          sh 'ssh  ubuntu@172.31.93.186 docker rm java-web-app || true'
          sh 'ssh  ubuntu@172.31.93.186 docker rmi -f  $(docker images -q) || true'
          sh "ssh  ubuntu@172.31.93.186 ${dockerRun}"
       }
